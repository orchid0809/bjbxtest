...
function selectAll(obj) {
	...
	if (status == "checked") {
		jQuery("#ListForm").find(":checkbox[id!=all]").attr("checked",
				"checked");
	} else {
...
function cmd() {
	...
	var mulitId = "";
	var id = arguments[1];
	jQuery("#ListForm").find(":checkbox:checked[name!='currentAll']").each(
			function() {
				if (jQuery(this).val() != "") {
...
function cmd_goods_list() {
	...
	if (checkall == "checked") {
		...
		jQuery("#ListForm").find("input[name='ids']").each(
				function() {
					if (jQuery(this).attr("checked") == "checked"
							&& jQuery(this).val() != "") {
						mulitId += jQuery(this).val() + ",";
		...
		if (confirm("此操作将耗费一定时间，确定要执行该操作？")) {
			...
			jQuery.ajax({
				...
				beforeSend : function() {
					...
					showDialog("msg_info", "操作中", "操作完成后点击确认关闭该窗口", 2,
							"succeed");
			...
			if (confirm("确定要执行该操作？")) {
				...
				jQuery
						.ajax({
							...
							beforeSend : function() {
								runcallback(confirm_action);
								showDialog("msg_info", "操作中", "操作完成后点击确认关闭该窗口",
										2, "succeed");
								jQuery("#ok").attr("disabled", "disabled");
								jQuery("#ok").attr("style",
							...
							success : function(data) {
								jQuery("input[name='inputscount']").val("");
								jQuery("#ListForm").find(":checkbox").attr(
										"checked", false);
								jQuery("#ok").removeAttr("disabled");
								jQuery("#ok").attr("style", "");
							}
...
function showDialog() {
	...
	var s = "<div id='"
			...
			+ id
			+ "\").remove();'>X</a><div><div class='message_white_iframe'><h3 class='message_white_title'><span>"
			+ title
	...
	var c = "<div id='"
			...
			+ "'><div class='message_white_content'> <a href='javascript:void(0);' class='white_close' onclick='javascript:jQuery(\"#"
			+ id
			+ "\").remove();'>X</a><div ><div class='message_white_iframe_del'><h3 class='message_white_title'><span>"
			+ title
	...
	var t = "<div id='"
			...
			+ "'></span><span class='message_white_font' style='font-size:14px;'>"
			+ content
			+ "</span></div>   <div class='message_white_box2'><input id='ok' type='button' value='确定'/></div></div></div></div><div class='black_overlay'></div>";
	if (type == 0) {// 消息框
	...
	if (confirm_action == undefined || confirm_action == "") {
		...
		if (back_function_args == undefined || back_function_args == "") {
			alert_timer_id = window.setInterval("closewin('" + id + "',"
					+ confirm_action + ")", 1000);
		} else {
			alert_timer_id = window.setInterval("closewin('" + id + "',"
					+ confirm_action + ",'" + back_function_args + "')", 1000);
		}
	}
	// 点击确定执行回调
...
function open_im() {
	...
	if (type == "user") {
		if (service_type) {
			window
					.open(
							url + "/chatting_distribute.htm?goods_id="
									+ goods_id + "&service_type="
									+ service_type + "&store_id=" + store_id
			...
			window
					.open(
							url + "/chatting_index.htm?goods_id=" + goods_id
									+ "&store_id=" + store_id,
							'',
							'height=660,width=1000,top=200,left=400,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
		}
...
jQuery(document)
		.ready(
				function() {
					...
					tipStyle();
					// 单独弹出一个编辑窗口
					jQuery("a[dialog_uri],input[dialog_uri],dt[dialog_uri]")
							.live(
									"click",
									function(e) {
										...
										if (dialog_uri != "undefined") {
											jQuery("body")
													.append(
															"<div id='"
																	+ dialog_id
																	+ "'><div class='white_content'> <a href='javascript:void(0);' dialog_uri='undefined' class='white_close' onclick='javascript:jQuery(\"#"
																	+ dialog_id
																	+ "\").remove();'>X</a><div class='white_box'><h1>"
																	+ dialog_title
					...
					jQuery(
							"a[ck_dialog_uri],input[ck_dialog_uri],dt[ck_dialog_uri]")
							.live(
									"click",
									function(e) {
										var mulitId = "";
										...
										if (dialog_uri != undefined
											...
											if (mulitId != "") {
												...
												jQuery('.black_overlay').css(
														"height", h);
												var dialog_left = (jQuery(
														document).width() - dialog_width) / 2;
