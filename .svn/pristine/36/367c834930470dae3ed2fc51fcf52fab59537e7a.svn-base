...
public class SpareGoodsManageAction {
		...
		for (String id : ids) {
			if (!id.equals("")) {
				...
				List<SpareGoodsClass> sgcs = this.sparegoodsclassService
						.query("select obj from SpareGoodsClass obj where obj.id in (:ids) order by obj.level desc",params, -1, -1);
	...
	public ModelAndView sparegoods_class_data(HttpServletRequest request,
		...
		List gcs = this.sparegoodsclassService.query("select obj from SpareGoodsClass obj where obj.parent.id =:pid",map, -1, -1);
	...
	public void sparegoods_class_verify(HttpServletRequest request,
		...
		if (pid.equals("")) {
			sgcs = this.sparegoodsclassService.query("select obj from SpareGoodsClass obj where obj.className=:className",params, -1, -1);
		} else {
			params.put("pid", CommUtil.null2Long(pid));
			sgcs = this.sparegoodsclassService.query("select obj from SpareGoodsClass obj where obj.className=:className and obj.parent.id =:pid",params, -1, -1);
		}
		if (sgcs.size() > 0) {
			ret = false;
		}
