...
public class BaseManageAction {
	...
	public void login_success(HttpServletRequest request,HttpServletResponse response) throws IOException {
		...
		if ((this.configService.getSysConfig().isIntegral())&& 
		   ((user.getLoginDate() == null) || (user.getLoginDate().before(CommUtil.formatDate(CommUtil.formatShortDate(new Date())))))) {
			user.setIntegral(user.getIntegral()+ this.configService.getSysConfig().getMemberDayLogin());
			IntegralLog log = new IntegralLog();
			log.setAddTime(new Date());
			log.setContent("用户" + CommUtil.formatLongDate(new Date()) + "登录增加"+ this.configService.getSysConfig().getMemberDayLogin()+ "分");
		...
		if (this.configService.getSysConfig().isSecond_domain_open()) {
			Cookie shopping_user_session = new Cookie("shopping_user_session",
					user.getId().toString());
	...
	public void logout_success(HttpServletRequest request,HttpServletResponse response) throws IOException {
		 ...
		 * ).getAttribute("shopping_view_type")); //跳转到微信端 if
		 * ((shopping_view_type != null) && (!shopping_view_type.equals("")) &&
	...
	public ModelAndView sys_config_save(HttpServletRequest request,HttpServletResponse response, String id, String list_url,String op_title) {
		...
		String uploadFilePath = this.configService.getSysConfig()
				.getUploadFilePath();
		String saveFilePathName = request.getSession().getServletContext().getRealPath("/")+ uploadFilePath + File.separator + "system";
		Map map = new HashMap();
		try {
			String fileName = this.configService.getSysConfig().getWebsiteLogo() == null ? "" : this.configService
