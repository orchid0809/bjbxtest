...
//----------------会员列表----------------
   ...
   //会员信息
   @SecurityMapping(display = false, rsequence = 0, title="会员信息", value="/admin/user_info.htm*", rtype="admin", rname="会员信息", rcode="user_info", rgroup="会员")
   @RequestMapping({"/admin/user_info.htm"})
   public ModelAndView user_info(HttpServletRequest request, HttpServletResponse response, String currentPage, String orderBy, String orderType, String province, String city, String mobile) {
     ModelAndView mv = new JModelAndView("admin/blue/user_info.html", 
       this.configService.getSysConfig(), this.userConfigService
       .getUserConfig(), 0, request, response);
     UserQueryObject uqo = new UserQueryObject(currentPage, mv, orderBy, 
    	       orderType);
    	     WebForm wf = new WebForm();
    	     wf.toQueryPo(request, uqo, User.class, mv);
    	     uqo.addQuery("obj.userRole", new SysMap("userRole", "BUYER"), "=");   	     
    	     if(province != null && !province.equals("")){
	    		 if(city != null && !city.equals("")){
	    			 uqo.addQuery("obj.area.parent.areaName", new SysMap("areaName", getArea(city) ), "=");
	    			 
	    			 uqo.addQuery("obj.area.parent.parent.areaName", new SysMap("areaName", getArea(province) ), "=", "and");
	    		 }else{
	    			 uqo.addQuery("obj.area.parent.parent.areaName", new SysMap("areaName", getArea(province) ), "=");
	    		 }
	    	 }
    	     if(mobile != null && !mobile.equals("")){
    	    	 uqo.addQuery("obj.mobile", new SysMap("mobile" , mobile ), "=");     	     	    	 
    	     }
    	     uqo.addQuery("obj.parent.id is null", null);
    	     IPageList pList = this.userService.list(uqo);
    	     String url = this.configService.getSysConfig().getAddress();
    	     ...
    	     CommUtil.saveIPageList2ModelAndView(url + "/admin/user_info.htm", "", 
    	       "", pList, mv);
    	     List areas = this.areaService.query(
  	    	       "select a from Area a where a.parent.id is null", null, 
  	    	       -1, -1);
   ...
   //搜索会员信息获取下级地区列表
   @SecurityMapping(display = false, rsequence = 0, title="搜索会员信息获取下级地区ajax", value="/admin/user_getAreaChilds.htm*", rtype="admin", rname="会员信息", rcode="user_info", rgroup="会员")
   @RequestMapping({"/admin/user_getAreaChilds.htm"})
   public ModelAndView user_getAreaChilds(HttpServletRequest request, HttpServletResponse response, String parent_id) {
     ModelAndView mv = new JModelAndView("admin/blue/user_info_area_chlids.html", 
...
//------------------会员删除----------------  
   ...
   public ModelAndView user_msg_send(HttpServletRequest request, HttpServletResponse response, String type, String list_url, String users, String grades, String title,String content) throws IOException {
     ModelAndView mv = new JModelAndView("admin/blue/success.html", 
       this.configService.getSysConfig(), this.userConfigService
       .getUserConfig(), 0, request, response);
